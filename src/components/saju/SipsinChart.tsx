    ...
    <div className="rounded-xl border border-border bg-card p-4">
      ...
      <div className="space-y-3">
        {SIPSIN_GROUPS.map((group) => {
          ...
          return (
            <div key={group.key} className="grid grid-cols-[5rem_1fr_3rem] items-center gap-3">
              {/* 라벨 */}
              <div className="text-right">
                <span className="text-xs font-semibold text-ink">{group.label}</span>
                <span className="block text-[10px] text-muted-foreground">{group.sub}</span>
              </div>
              ...
              <div className="text-left">
                <span className="text-xs font-bold text-ink">{count}개</span>
      ...
      {/* 설명 범례 */}
      <div className="mt-4 pt-3 border-t border-border grid grid-cols-1 sm:grid-cols-2 gap-1.5">
        {SIPSIN_GROUPS.map((group) => {
          const count = counts[group.key] ?? 0;
          return (
